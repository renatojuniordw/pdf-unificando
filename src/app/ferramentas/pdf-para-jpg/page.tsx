...
export default function PdfParaJpgPage() {
  ...
  const handleDrop = useCallback((files: File[]) => process(files[0], { dpi }), [process, dpi])
          ...
          {status === 'idle' && (
            <>
              <div className="border-4 border-slate-950 bg-white shadow-[8px_8px_0px_#000] p-6">
                <p className="text-xs font-black uppercase tracking-widest mb-3">RESOLUÇÃO (DPI)</p>
                <div className="flex gap-3">
                  {DPI_OPTIONS.map(d => (
                    <button key={d} onClick={() => setDpi(d)} className={`flex-1 border-4 p-3 font-black uppercase text-sm tracking-widest transition-all ${dpi===d ? 'bg-slate-950 text-[#ccff00] border-slate-950 shadow-[4px_4px_0px_#ccff00]' : 'bg-white text-slate-950 border-slate-950 shadow-[2px_2px_0px_#000] hover:bg-slate-100'}`}>{d} DPI</button>
                  ))}
                </div>
              </div>
