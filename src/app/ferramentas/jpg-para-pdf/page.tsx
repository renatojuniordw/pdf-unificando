...
export default function JpgParaPdfPage() {
  ...
  const handleDrop = useCallback((dropped: File[]) => {
    setFiles(prev => [...prev, ...dropped.map(f => ({ id: `${f.name}-${Date.now()}-${Math.random()}`, file: f }))])
  }, [])
  const handleProcess = useCallback(() => process(files.map(f => f.file)), [process, files])
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 py-12">
        <div className="max-w-2xl flex flex-col gap-6">
          {status === 'idle' && (
            <>
              <DropZone accept={{'image/jpeg':['.jpg','.jpeg'],'image/png':['.png']}} multiple onDrop={handleDrop} />
              {files.length > 0 && (
                <>
                  <FileQueue files={files} onReorder={setFiles} onRemove={(id) => setFiles(prev => prev.filter(f => f.id !== id))} />
                  <button onClick={handleProcess} className="w-full bg-slate-950 text-[#ccff00] border-4 border-slate-950 shadow-[8px_8px_0px_#ccff00] px-8 py-5 font-black uppercase tracking-[0.2em] hover:bg-slate-800 hover:-translate-y-1 transition-all">
          ...
          {status==='rate_limited' && <RetryCountdown secondsLeft={secondsLeft} progress={progress} onRetry={handleReset}/>}
          {status==='error' && (
          ...
          {status==='done' && downloadUrl && <DownloadButton url={downloadUrl} filename={outputName!} fileSize={processedSize} onReset={handleReset}/>}
        </div>
      </div>
