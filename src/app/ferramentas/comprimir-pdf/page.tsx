...
export default function ComprirPdfPage() {
  ...
  const { status, error, downloadUrl, outputName, originalSize, processedSize, process, reset, secondsLeft, progress } = useFileProcessor({
        ...
        <div className="max-w-2xl">
          {status === 'idle' && (
            <div className="flex flex-col gap-6">
              <div className="border-4 border-slate-950 bg-white shadow-[8px_8px_0px_#000] p-6">
                <p className="text-xs font-black uppercase tracking-widest mb-4">QUALIDADE</p>
          ...
          {status === 'done' && downloadUrl && (
            <div className="flex flex-col gap-6">
              ...
              <DownloadButton url={downloadUrl} filename={outputName!} fileSize={processedSize} onReset={reset} />
            </div>
          )}
        </div>
      </div>
