...
export default function RodarPdfPage() {
  ...
  const handleDrop = useCallback((files: File[]) => process(files[0], { angle }), [process, angle])
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 py-12">
        <div className="max-w-2xl flex flex-col gap-6">
          {status === 'idle' && (
            <>
              <div className="border-4 border-slate-950 bg-white shadow-[8px_8px_0px_#000] p-6">
                <p className="text-xs font-black uppercase tracking-widest mb-3">ÂNGULO DE ROTAÇÃO</p>
                <div className="flex gap-3">
                  {ANGLES.map(a => (
                    <button key={a} onClick={() => setAngle(a)} className={`flex-1 border-4 p-3 font-black uppercase text-sm tracking-widest transition-all ${angle===a ? 'bg-slate-950 text-[#ccff00] border-slate-950 shadow-[4px_4px_0px_#ccff00]' : 'bg-white text-slate-950 border-slate-950 shadow-[2px_2px_0px_#000] hover:bg-slate-100'}`}>{a}°</button>
                  ))}
                </div>
              </div>
          ...
          {status==='rate_limited' && <RetryCountdown secondsLeft={secondsLeft} progress={progress} onRetry={reset}/>}
          {status==='error' && (
            <div className="bg-[#ff4d4d] text-white border-4 border-slate-950 shadow-[4px_4px_0px_#000] p-6 flex items-center gap-4">
              ...
              <button onClick={reset} className="ml-auto border-2 border-white px-4 py-2 font-black uppercase text-xs">TENTAR NOVAMENTE</button>
            </div>
          )}
          {status==='done' && downloadUrl && <DownloadButton url={downloadUrl} filename={outputName!} fileSize={processedSize} onReset={reset}/>}
        </div>
      </div>
